...
- 코드 셀 아래 실행 결과를 보려면 `executed/` 경로의 노트북을 확인하면 됩니다.

## Auto-Generated Executed Notebooks

- 원본 노트북(`Pre-algebra/...`)은 clean 상태(출력 없음)로 유지됩니다.
- `push` 시 GitHub Actions가 원본을 실행하고, 결과가 포함된 노트북을 `executed/`에 자동 반영합니다.
- Executed Lesson 1: `executed/Pre-algebra/unit_8/Lesson1_percent_word_problems.ipynb`
- Executed Lesson 2: `executed/Pre-algebra/unit_8/Lesson2_rational_number_word_problems.ipynb`
...
커밋 정책:
- `.ipynb`는 pre-commit 훅에서 자동으로 `outputs`/`execution_count`가 제거됩니다.
- 실수로 남아도 GitHub Actions에서 검증 실패로 차단됩니다.
